"""UI helpers and GTK-dependent utilities.

Pure business logic has been extracted to faugus/core/utils.py.
This module retains only GTK-dependent functions and re-exports
the pure functions for backward compatibility.
"""
...
# Existing code does ``from faugus.utils import *`` so these must
# remain importable from this module.
from faugus.core.utils import (  # noqa: F401
    ensure_parent_dir,
    load_json_file,
    save_json_file,
    load_recents,
...
# ---------------------------------------------------------------------------
# GTK-dependent functions (UI layer)
# ---------------------------------------------------------------------------
...
def update_games_json():
    ...
    Uses paths from path_manager module.
    """
    from faugus.core.utils import update_games_json as _update
    icons_dir = PathManager.user_config('faugus-launcher/icons')
    _update(games_json, icons_dir)

def populate_combobox_with_runners(combobox):
    from faugus.core.utils import build_runner_list
    runners = build_runner_list(proton_cachyos, compatibility_dir)
